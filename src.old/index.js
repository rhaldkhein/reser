...
export function connectWithModules(mapStates, mapDispatch) {
  return function (ChildComponent, ...moduleNames) {
    return connect(
      state => {
        let mapped = mapStates(state)
        mapped.state = mapStateToProps(moduleNames, state)
        return mapped
      },
      mapDispatch
    )(createComponentWithModules(ChildComponent, moduleNames))
...
export function connectWithRequiredModules(mapStates, mapDispatch) {
  return function (ChildComponent, ...moduleNames) {
    return connect(
      state => {
        let mapped = mapStates(state)
